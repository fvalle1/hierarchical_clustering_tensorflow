            ...
            for i in range(self.steps - 1):
                ...
                self.nddistances = tf.reshape(
                    tf.boolean_mask(self.distances,
                                    tf.logical_not(tf.equal(self.distances, tf.zeros_like(self.distances)))),
                    (self.n, self.n - 1))  # 1 is diagonal
                self.actual_minimums = \
                    tf.sort(tf.sort(tf.where(tf.equal(tf.reduce_min(self.nddistances), self.distances)), axis=1),
                            axis=0,
                            name="assignemts")[0]
                ...
                self.data = tf.concat([self.data, [self.new_pos]], axis=0)
                ...
                self.new_data = tf.Variable(tf.zeros((self.n_actual_clusters, self.data.shape[1]), dtype=tf.float64),
                                            dtype=tf.float64)
                ...
                self.Z.append(
                    tf.stack([self.original_cluster_indexes[0], self.original_cluster_indexes[1], self.min_distance,
                              self.current_size],
                             0).eval())
